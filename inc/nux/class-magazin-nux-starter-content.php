<?php
/**
 * magazin NUX Starter Content Class
 *
 * @package  magazin
 * @since    2.0.0
 */
...
if ( ! class_exists( 'magazin_NUX_Starter_Content' ) ) :

	/**
	 * The magazin NUX Starter Content class
	 */
	class magazin_NUX_Starter_Content {
		/**
		 * Setup class.
		 *
		 * @since 2.2.0
		...
		public function __construct() {
			...
			if ( ! isset( $_GET['sf_starter_content'] ) || 1 !== absint( $_GET['sf_starter_content'] ) ) { // WPCS: input var ok.
				add_filter( 'magazin_starter_content', '__return_empty_array' );
			}
		}

		/**
		 * Remove default widgets on activation
		 * Set an option so that this is only done the first time the user activates magazin.
		 *
		 * @since 2.2.0
		 * @return void
		 */
		public function remove_default_widgets() {
			if ( false === (bool) get_option( 'magazin_cleared_widgets' ) && true === (bool) get_option( 'magazin_nux_fresh_site' ) ) {
				update_option( 'sidebars_widgets', array( 'wp_inactive_widgets' => array() ) );
				update_option( 'magazin_cleared_widgets', true );
			}
		}
		...
		public function starter_content() {
			$starter_content = array(
				'posts'       => array(
					'about'   => array(
						'post_type'    => 'page',
						'post_title'   => __( 'About', 'magazin' ),
						'post_content' => __( 'You might be an artist who would like to introduce yourself and your work here or maybe you&rsquo;re a business with a mission to describe.', 'magazin' ),
					),
					'contact' => array(
						'post_type'    => 'page',
						'post_title'   => __( 'Contact', 'magazin' ),
						'post_content' => __( 'This is a page with some basic contact information, such as an address and phone number. You might also try a plugin to add a contact form.', 'magazin' ),
					),
					'blog',
				),
				'attachments' => array(
				...
				'nav_menus'   => array(
					...
					'secondary' => array(
						'name'  => __( 'Secondary Menu', 'magazin' ),
						'items' => array(
							'my_account' => array(
								'type'      => 'post_type',
								'object'    => 'page',
			...
			if ( version_compare( get_bloginfo( 'version' ), '5.2', '>=' ) &&
			   ( defined( 'WC_VERSION' ) && version_compare( WC_VERSION, '3.6.0', '>=' ) ) ) {
				$homepage_content = array(
					'post_title' => esc_attr__( 'Homepage', 'magazin' ),
					'template'   => 'template-fullwidth.php',
				);
			} else {
				$homepage_content = array(
					'post_title'   => esc_attr__( 'Welcome', 'magazin' ),
					/* translators: %s: 'End Of Line' symbol */
					'post_content' => sprintf( esc_attr__( 'This is your homepage which is what most visitors will see when they first visit your shop.%sYou can change this text by editing the "Welcome" page via the "Pages" menu in your dashboard.', 'magazin' ), PHP_EOL . PHP_EOL ),
			...
			$woocommerce_pages        = magazin_NUX_Admin::get_woocommerce_pages();
			...
			add_theme_support( 'starter-content', apply_filters( 'magazin_starter_content', $starter_content ) );
		}
		...
		public function filter_start_content( $content, $config ) {
			...
			if ( true !== (bool) get_option( 'magazin_nux_fresh_site' ) ) {
				unset( $content['posts']['about'] );
				unset( $content['posts']['contact'] );
				unset( $content['nav_menus'] );
				unset( $content['widgets'] );
		...
		public function add_product_tax() {
			...
			add_filter( 'magazin_product_categories_shortcode_args', array( $this, 'filter_sf_categories' ) );
		}
		...
		/**
		 * Filter magazin Product Categories shortcode.
		 *
		 * @since 2.2.0
		...
		private function _starter_content_products() {
			$accessories_name        = esc_attr__( 'Accessories', 'magazin' );
			$accessories_description = esc_attr__( 'A short category description', 'magazin' );

			$hoodies_name        = esc_attr__( 'Hoodies', 'magazin' );
			$hoodies_description = esc_attr__( 'A short category description', 'magazin' );

			$tshirts_name        = esc_attr__( 'Tshirts', 'magazin' );
			$tshirts_description = esc_attr__( 'A short category description', 'magazin' );

			$products = array(
				// Accessories.
				'beanie'             => array(
					'post_title'     => esc_attr__( 'Beanie', 'magazin' ),
				...
				'belt'               => array(
					'post_title'     => esc_attr__( 'Belt', 'magazin' ),
					'post_content'   => 'Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo.',
					'post_type'      => 'product',
					'comment_status' => 'open',
				...
				'cap'                => array(
					'post_title'     => esc_attr__( 'Cap', 'magazin' ),
					'post_content'   => 'Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo.',
					'post_type'      => 'product',
					'comment_status' => 'open',
					'thumbnail'      => '{{cap-image}}',
				...
				'sunglasses'         => array(
					'post_title'     => esc_attr__( 'Sunglasses', 'magazin' ),
					'post_content'   => 'Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo.',
					'post_type'      => 'product',
					'comment_status' => 'open',
				...
				'hoodie-with-logo'   => array(
					'post_title'     => esc_attr__( 'Hoodie with Logo', 'magazin' ),
					'post_content'   => 'Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo.',
					'post_type'      => 'product',
					'comment_status' => 'open',
				...
				'hoodie-with-pocket' => array(
					'post_title'     => esc_attr__( 'Hoodie with Pocket', 'magazin' ),
					'post_content'   => 'Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo.',
					'post_type'      => 'product',
					'comment_status' => 'open',
				...
				'hoodie-with-zipper' => array(
					'post_title'     => esc_attr__( 'Hoodie with Zipper', 'magazin' ),
					'post_content'   => 'Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo.',
					'post_type'      => 'product',
					'comment_status' => 'open',
				...
				'hoodie'             => array(
					'post_title'     => esc_attr__( 'Hoodie', 'magazin' ),
					'post_content'   => 'Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo.',
					'post_type'      => 'product',
					'comment_status' => 'open',
				...
				'long-sleeve-tee'    => array(
					'post_title'     => esc_attr__( 'Long Sleeve Tee', 'magazin' ),
					'post_content'   => 'Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo.',
					'post_type'      => 'product',
					'comment_status' => 'open',
				...
				'polo'               => array(
					'post_title'     => esc_attr__( 'Polo', 'magazin' ),
					'post_content'   => 'Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo.',
					'post_type'      => 'product',
					'comment_status' => 'open',
				...
				'tshirt'             => array(
					'post_title'     => esc_attr__( 'Tshirt', 'magazin' ),
					'post_content'   => 'Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo.',
					'post_type'      => 'product',
					'comment_status' => 'open',
				...
				'vneck-tee'          => array(
					'post_title'     => esc_attr__( 'Vneck Tshirt', 'magazin' ),
					'post_content'   => 'Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. Aenean ultricies mi vitae est. Mauris placerat eleifend leo.',
					'post_type'      => 'product',
					'comment_status' => 'open',
			...
			return apply_filters( 'magazin_starter_content_products', $products );
		}
		...
		private function _homepage_blocks_content() {
			$content = '
				...
				<h2 style="text-align:center">' . __( 'Shop by Category', 'magazin' ) . '</h2>
				<!-- /wp:heading -->
				...
				<!-- wp:heading {"align":"center"} -->
				<h2 style="text-align:center">' . __( 'New In', 'magazin' ) . '</h2>
				<!-- /wp:heading -->
				...
				<!-- wp:heading {"align":"center"} -->
				<h2 style="text-align:center">' . __( 'Fan Favorites', 'magazin' ) . '</h2>
				<!-- /wp:heading -->
				...
				<!-- wp:heading {"align":"center"} -->
				<h2 style="text-align:center">' . __( 'On Sale', 'magazin' ) . '</h2>
				<!-- /wp:heading -->
				...
				<!-- wp:heading {"align":"center"} -->
				<h2 style="text-align:center">' . __( 'Best Sellers', 'magazin' ) . '</h2>
				<!-- /wp:heading -->
		...
		private function _replace_homepage_blocks_symbols() {
			...
			if ( ! empty( $hero ) ) {
				$cover = '
					...
					<h1 style="text-align:center">' . __( 'Welcome', 'magazin' ) . '</h1>
					<!-- /wp:heading -->

					<!-- wp:paragraph {"align":"center","customTextColor":"#000000"} -->
					<p style="color:#000000;text-align:center" class="has-text-color">' . __( 'This is your homepage which is what most visitors will see when they first visit your shop.', 'magazin' ) . '</p>
					<!-- /wp:paragraph -->

					<!-- wp:paragraph {"align":"center","customTextColor":"#000000"} -->
					<p style="color:#000000;text-align:center" class="has-text-color">' . __( 'You can change this text by editing the "Welcome" page via the "Pages" menu in your dashboard.', 'magazin' ) . '</p>
			...
			if ( ! empty( $products ) ) {
				$handpicked = '
					<!-- wp:heading {"align":"center"} -->
					<h2 style="text-align:center">' . __( 'We Recommend', 'magazin' ) . '</h2>
					<!-- /wp:heading -->
		...
		private function _validate_tasks( $tasks ) {
			$valid_tasks = apply_filters( 'magazin_valid_tour_tasks', array( 'homepage', 'products' ) );
...
return new magazin_NUX_Starter_Content();
