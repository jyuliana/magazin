...
magazin is the perfect theme for your next WooCommerce project.

== Description ==

magazin is the perfect theme for your next WooCommerce project. Designed and developed by WooCommerce Core developers, it features a bespoke integration with WooCommerce itself plus many of the most popular customer facing WooCommerce extensions. There are several layout & color options to personalize your shop, multiple widget regions, a responsive design and much more. Developers will love its lean and extensible codebase making it a joy to customize and extend. Looking for a WooCommerce theme? Look no further!

For more information about magazin please go to https://woocommerce.com/magazin/.

For even more customization, check out magazin extensions https://woocommerce.com/product-category/magazin-extensions/ and magazin child themes https://woocommerce.com/product-category/themes/magazin-child-theme-themes/.

== Installation ==

1. In your admin panel, go to Appearance -> Themes and click the 'Add New' button.
2. Type in magazin in the search form and press the 'Enter' key on your keyboard.
3. Click on the 'Activate' button to use your new theme right away.
4. Go to https://docs.woocommerce.com/documentation/themes/magazin/ guides on how to customize this theme.
...
magazin is based on Underscores https://underscores.me/, (C) 2012-2017 Automattic, Inc.
...
magazin serves fonts via the hosted Google Fonts service. magazin does not share any data with Google directly.
...
* Fix - Make magazin admin notices conform to new WP.org theme rules. #1190
...
= 2.4.3 - 2019-02-05 =
* Feature - Replicate magazin's layout logic in the block editor. Wide and Full-width options are made available only if supported.
* Enhancement - Product pagination is now circular and skips hidden products.
* Tweak - Remove unused variables from `inc/customizer/class-magazin-customizer.php`.
* Tweak - Refactored "Posted on" section in `magazin_post_meta()`.
* Tweak - Standardize mixing of static/non static function calls in the `magazin_Customizer` class.
...
* Fix - When outputting custom styling for extensions, check if the object `$magazin` is in the expected format and has right properties.
* Dev - Babel added as a dependency.
* Dev - New `assets/js/src/editor.js` file.
* Dev - Revert filter name. `magazin_woocommerce_customizer_css` reverted to `magazin_customizer_woocommerce_css`.
* Dev - New hooks added to post header section: `magazin_post_header_before`, `magazin_post_header_after`.

= 2.4.2 - 2018-12-11 =
* Fix - Load in `functions.php` missing `magazin-woocommerce-functions.php` file required for compatibility with Jetpack's Infinite Scroll feature.
...
* Fix - Moved margin from `.woocommerce-breadcrumb` to the new `.magazin-breadcrumb` wrapper.
...
= 2.2.7 - 2018-01-23 =
* Fix - Reverted Chromium overflow fix introduced in 2.2.6 due to incompatibility with other magazin products.
* Fix - Jetpack Google Translate widget styling.
* Tweak - Improved Grouped products table styling.
* Dev - Added filters; `magazin_custom_logo_args`, `magazin_register_nav_menus`, `magazin_html5_args`, and `magazin_site_logo_args`.
...
* Fix - Fixed typo in the `magazin_WooCommerce` class, `thumbnail_columns` method description. Kudos [@cryptexvinci](https://github.com/cryptexvinci).
...
* Dev - Removed sticky cart. See [this issue](https://github.com/woocommerce/magazin/issues/637) for more information.
...
* Dev - New hook to filter sidebar regions: `magazin_sidebar_args`.
...
* Tweak - Removed Structured Data. See [this comment](https://github.com/woocommerce/magazin/issues/620#issuecomment-313629032).
...
* Dev - New hook after body tag: `magazin_before_site`.
...
* Dev - Added actions; `magazin_jetpack_infinite_scroll_before`, `magazin_jetpack_infinite_scroll_after`, `magazin_jetpack_product_infinite_scroll_before` and `magazin_jetpack_product_infinite_scroll_after`.
* Dev - Added filters; `magazin_footer_widget_rows` and `magazin_footer_widget_columns` to easily control the number of widget rows / columns in the footer.
...
* Dev - Upsells columns filterable via `magazin_upsells_columns`.
* Dev - Added a column wrapper to product loops. To adjust the layout you now only need to filter `magazin_loop_columns`.
...
* Dev - Refactored `magazin_get_content_background_color()` to account for and give priority (over magazin Designer) to magazin Powerpack.
* Dev - Deprecated `is_woocommerce_activated()` and made it pluggable.
* Dev - Added `magazin_is_woocommerce_activated()`.
...
* Dev - `magazin_post_thumbnail()` is now hooked in to `magazin_post_content_before`.
* Dev - Added `magazin_post_content_before` and `magazin_post_content_after` actions.
* Dev - `magazin_post_thumbnail()` now provides a default size.
...
* Dev - Restructured the way objects are initiated. Objects are now initiated into an accessible global $magazin for access to hooks. Kudos [@jtsternberg](https://github.com/jtsternberg).
...
* Dev - Made `magazin_primary_navigation_wrapper()` and `magazin_primary_navigation_wrapper_close()` pluggable.
...
* Dev - Deprecate `magazin_html_tag_schema()`.

= 2.0.3 - 2016-06-02 =
* Fix - Fatal error when using magazin without WooCommerce.
...
* Dev - Added `magazin_sticky_order_review` filter.
* Dev - Added `magazin_structured_data` filter for structured data customisation.
...
* Dev - Apply Customizer setting defaults on `customize_register` to account for child themes filtering `magazin_setting_default_values`.
* Dev - Fix typo in the `magazin_homepage_after_best_selling_products` action name.
...
* Feature - oEmbed styling to match magazin aesthetic.
* Feature - The welcome screen now dynamically pulls in new products.
* Feature - Added `magazin_header_styles()` for the header image/styles along with the `magazin_header_styles` filter to customise the treatment of header styles.
...
* Dev - Deprecated `magazin_categorized_blog()`.
* Dev - Deprecated `magazin_sanitize_hex_color()`.
* Dev - Deprecated `magazin_sanitize_layout()`.
...
= 1.6.0 - 2016-01-05 =
* Feature - [magazin Mega Menus](https://woocommerce.com/products/magazin-mega-menus/) integration.
...
* Tweak - Added new function `magazin_star_rating_script`. Outputs JavaScript for the new star rating input while we wait for WooCommerce 2.5 to be released.
...
* Dev - Added some new hooks to the homepage template tags; `magazin_homepage_after_product_categories_title`, `magazin_homepage_after_recent_products_title`, `magazin_homepage_after_featured_products_title`, `magazin_homepage_after_popular_products_title`, `magazin_homepage_after_on_sale_products_title`
* Dev - Styling for 6 column product layouts.
* Dev - Updates FontAwesome to version 4.5.0.

= 1.5.3 - 2015-11-13 =
* Fix - magazin will now automatically load a child themes stylesheet.
...
* Dev - `magazin_display_comments` is now hooked into `magazin_single_post_after` with priority 20 (was 10).
* Localization - All translations are now managed on [WordPress.org](https://translate.wordpress.org/projects/wp-themes/magazin)
...
* Dev - The default layout is now  filterable via `magazin_default_layout`.
...
* Dev - Added `magazin_sanitize_checkbox()` sanitization function.
* Dev - Added `magazin_Custom_Radio_Image_Control` class for creating radio image controls in the Customizer.
* Dev - Added `magazin_post_thumbnail()`.
* Dev - Renamed `do_shortcode_func()` to `magazin_do_shortcode()`.
...
* Dev - `$magazin_version` global when using a child theme.

= 1.4.5 - 2015-05-14 =
* Fix - Use the correct escaping function in `magazin_product_categories()`.
...
* Dev - Made `Layout_Picker_magazin_Control` class pluggable. Props @niravmehta
...
* Tweak - Skip links are now a function (`magazin_skip_links()`) hooked into `magazin_header`.
...
* Dev - Fixed typo in classname: `magazin-feautred-products` is now `magazin-featured-products`.
...
* Dev - Added `magazin_related_products_args` filter.
...
* Fix - `magazin_menu_toggle_text` filter. Props [jesinwp](https://github.com/jesinwp).
...
* Tweak - `mark` styling.
* Dev - Added `magazin_sanitize_choices()`.
...
* Dev - `magazin_header_cart()` is now pluggable.
...
* Dev - Added `magazin_menu_toggle_text` filter.
* Dev - Updated how WooCommerce styles are dequeued.

= 1.2.3 - 2015-01-07 =
* Fix - Button border in WooCommerce messages when using the magazin Designer extension.
* Tweak - Stock icon.
...
* Dev - Made `magazin_product_search()` pluggable.
...
* Dev - Added `magazin_single_post_posted_on_html` filter.
...
* Dev - Added `magazin_copyright_text` filter.
* Dev - Tweaked how magazin determines whether Customizer logic should be loaded.
* Dev - Added `magazin_after_footer` action.
...
* Dev - Added `magazin_product_thumbnail_columns` filter.
