...
( function( wp, $ ) {
	...
	api.SFGuidedTour = {
		...
		_hideTour: function( remove ) {
			...
			$( 'body' ).addClass( 'sf-exiting' ).on( 'animationend.magazin webkitAnimationEnd.magazin', function() {
				$( this ).removeClass( 'sf-exiting' ).off( 'animationend.magazin webkitAnimationEnd.magazin' ).addClass( 'sf-hidden' );
				self.$container.hide();
		...
		_revealTour: function() {
			...
			$( 'body' ).addClass( 'sf-entering' ).on( 'animationend.magazin webkitAnimationEnd.magazin', function() {
				$( this ).removeClass( 'sf-entering' ).off( 'animationend.magazin webkitAnimationEnd.magazin' );
		...
		_moveContainer: function( $selector ) {
			...
			this.$container.addClass( 'sf-moving' ).css({ 'transform': 'translateY(' + parseInt( position, 10 ) + 'px)' }).on( 'transitionend.magazin', function() {
				self.$container.removeClass( 'sf-moving' );
				self.$container.off( 'transitionend.magazin' );
			} );
		},
